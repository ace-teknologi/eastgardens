...
def test_eastgardens_vanilla():
    ...
    response = eastgardens(
        {'Records': [{'cf': {'request': {'uri': '/thing'}}}]}, None)
...
def test_eastgardens_origin():
    ...
    response = eastgardens(
        {'Records': [{'cf': {'request': {'uri': '/thing', 'otherstuff': 'yeah'}}}]}, None)
    assert response == {'uri': '/thing', 'otherstuff': 'yeah'}


def test_eastgarders_redirects():
    variables.REDIRECTS = {'/a-hit': 'https://great-site.com/kitten-videos'}
    response = eastgardens(
        {'Records': [{'cf': {'request': {'uri': '/a-hit', 'otherstuff': 'yeah'}}}]}, None)
    assert response == {'headers': {'location': [
